...
| C | Query system — parser/AST, hledger query semantics, Python `re` extension, CLI/report routing | `[IN PROGRESS]` — Phase 1 done: `hledger-researcher`'s semantics brief (`core-redefinition/17-query-semantics-brief.md`) plus a standalone, tested `ledgerkit/query/` subpackage (AST, `HledgerRegex`-subset validation, parser, evaluator) implementing `acct:`/`desc:`/`date:`(simple)/`depth:`/`status:`/`not:`. Not yet wired into `reports.py`/`cli.py`/`Query`; `tag:`/`cur:`/smart-dates/`PythonRegex` extension deferred. 7 compat-register entries added, all `status: proposed`. **Phase 2 in progress**: CodeCompass-assisted query/report/CLI integration (`core-redefinition/18-stage-c-phase-2-codecompass-adoption-plan.md`) — commit boundary 1 (CodeCompass baseline) done. Commit boundary 2 done: `-q`/`--query` wired into `balance`/`register`/`accounts`/`stats` via a private, internal-only `reports._query_ast` parameter (no public API change — see the API-boundary decision gate, `knowledge/DECISIONS.md`). Differential-verified against the pinned hledger 1.52.4 binary; found and fixed two real CLI defects (`knowledge/EDGE_CASES.md` EC-016) and corrected a Stage C Phase 1 misclassification (`LK-COMPAT-QUERY-DEPTH-001`, now `intentional_divergence`, EC-017). Commit boundary 3 (context evaluation + closeout) next. | `core-redefinition/06`, `07`, `16`, `17`, `18` |
