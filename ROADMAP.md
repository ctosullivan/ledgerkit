...
| C | Query system — parser/AST, hledger query semantics, Python `re` extension, CLI/report routing | `[IN PROGRESS]` — Phase 1 scoping: `hledger-researcher` producing a semantics brief for the initial term set (`acct:`/`desc:`/`date:`(simple)/`depth:`/`status:`/`not:`) before any `ledgerkit/query/` code is written. | `core-redefinition/07` |
