...
### Tags (parsing/storage — Stage C Phase 4)

**Not yet queryable** — `tag:NAME[=REGEX]` itself is not implemented (see
Query Language section below); this section covers parsing and storage
only. Grounded in `dev-docs/planning/core-redefinition/
19-tag-query-semantics-brief.md`/`20-tag-parsing-syntax-brief.md`,
...
| Term | Example | Notes |
|---|---|---|
| `acct:REGEX` / bare pattern | `acct:food`, `food` | **[IMPLEMENTED]** Case-insensitive infix match against a posting's account name; bare pattern defaults to `acct:`. A transaction matches if any of its postings match. Differential-verified against hledger 1.52.4 (`LK-COMPAT-QUERY-ACCT-001`, `status: self-verified` — see `dev-docs/compat-register/README.md`'s Stage C Phase 5 note on the new verification-independence process). |
| `desc:REGEX` | `desc:amazon` | **[IMPLEMENTED]** Case-insensitive infix match against the transaction description; a posting inherits its transaction's match. Differential-verified (`LK-COMPAT-QUERY-DESC-001`, `status: self-verified`). |
...
| `tag:NAME[=REGEX]` | `tag:category=food` | Not implemented — Stage C follow-on work. Its prerequisite (tag parsing/storage — `Transaction.tags`/`Posting.tags`/`Journal.declared_account_tags`) is now `[IMPLEMENTED]`, see the Tags section above; only the query-matching layer itself (and its four inheritance rules, and its always-AND-never-OR combination behaviour — genuinely different from `acct:`/`desc:`/`status:`) remains to be built. |
...
**Regex dialect (`HledgerRegex`):** `acct:`/`desc:`/bare-pattern arguments
are validated against the hledger-compatible subset before use — literals,
...
| Tags — the `tag:` **query term** specifically | `tag:NAME[=REGEX]` | Not implemented — see Query Language section. Inline tag **parsing/storage** is now **[IMPLEMENTED]** (see Tags section above); only matching/querying remains out of scope. |
