...
**Stage C Phase 8**: `ledgerkit.models.Query` (the typed, `-q`-string-free
Python constructor documented in `dev-docs/api-spec.md`) now converges onto
this exact same `QueryNode` AST/evaluator via a new translator,
`ledgerkit.query.compat._query_to_ast` — it is no longer matched by a
separate, ad hoc, non-`HledgerRegex`-validated heuristic. `Query.account`/
`.not_account`/`.payee` are now `HledgerRegex`-validated exactly like
`acct:`/`desc:` below, **including the Stage C Phase 7 empty-pattern
rejection** — `Query(account="")` now raises `QueryParseError`, where
previously an empty pattern trivially matched everything (a plausible
accidental-caller mistake now surfaced loudly instead of silently). This
extends, via `ledgerkit.reports.balance_from_spec`, to `ReportSection.
accounts`/`.exclude` too (a public dataclass, `dev-docs/api-spec.md`) —
its own OR/exclude combination logic is unchanged, only its regex dialect
converged. `stats(query=Query(account=..., not_account=...))` also now
applies those two fields (previously silently ignored, a pre-existing
documented gap) — identical to what `-q "acct:..." stats` already did.
This is a disclosed, intentional, **breaking** change (not backward-
compatible), made while Ledgerkit is pre-`1.0.0` — see `dev-docs/
versioning.md` and `dev-docs/planning/core-redefinition/
27-query-shim-convergence-design.md` §5.2 for the complete list of every
behaviour change this phase makes, and the new `LK-COMPAT-QUERY-SHIM-001`
compat-register entry.
...
| `date:PERIODEXPR` | `date:2024-01-15`, `date:2024-01-01..2024-02-01` | **[IMPLEMENTED — simple dates only]** A single full date matches only that one day; a range's end date is **exclusive** (matching real hledger `date:` span semantics — differential-verified, `LK-COMPAT-QUERY-DATE-001`, `status: self-verified`). This differs from the existing `Query.date_to`, which is inclusive -- reconciled by `ledgerkit.query.compat._exclusive_end` (Stage C Phase 8), which adds one day to `Query.date_to` to produce the equivalent exclusive `DateSpan.end`, except at `datetime.date.max` (which has no representable successor and maps to `end=None`/unbounded instead of overflowing). hledger's smart/relative/period dates (`today`, `last month`, `2024Q1`, etc.) are **not** implemented — see Undecided/Future. |
...
An empty pattern (`""`) also raises (Stage C Phase 7, see the Query
Language section's `tag:` row for the full writeup). **Stage C Phase 9**:
a pattern containing an unescaped `|` with an empty branch on either
side — `a|`, `|a`, `(|)`, `a||b`, `(a|)`, `(|a)`, and 12 further patterns
in the same family — now also raises, matching real hledger's regex-tdfa
engine's own parse-time rejection exactly (a purely local,
single-character-adjacency, escape-aware rule; a `|` is "empty" on a
side when that side is pattern-start/-end or the nearest unescaped
neighbour is `(`, `)`, or another `|`). `()`, `(a)`, `a|b`, `(a|b)`,
`()|a`, `a|()`, anchor-only branches (`^|a`, `a|$`), and escaped
pipes/parens (`a\|\|b`, `\(|a`, `a|\)`) remain accepted — this is a
narrow, family-specific fix, not a broader alternation-semantics change.
Resolves `LK-MISMATCH-QUERY-REGEX-EMPTYALT-001`; documented at
`dev-docs/compat-register/LK-COMPAT-QUERY-REGEX-EMPTYALT-001.yaml`
(`status: proposed` as of this writing — independent verification by a
separately-dispatched `compat-differential-tester` is the required next
step before this can be promoted to `verified`).
