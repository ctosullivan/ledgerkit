...
| Account-type semantics | Not implemented (`Undecided/Future` in `hledger-compatibility.md`: "Account type inference from name prefixes") | No hard prerequisite — can start once `account` directive's `type:` tag is parsed (currently stripped/ignored). **Guardrail** (`06-core-architecture.md` §6.3, `16-model-review.md` §16.3): store the parsed type in a *new* field — never by changing `Journal.declared_accounts`'s `list[str]` shape, which `ledgerkit-editor` depends on. |
