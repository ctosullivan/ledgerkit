...
Adopted Stage C Phase 5A (`dev-docs/planning/core-redefinition/
04-codecompass-integration.md` §4.7 for the full routine workflow). The
table below this line is generated by `codecompass index`/`sync` —
**never hand-edit inside the marker block**; regenerate it instead.
Consult `.claude/skills/codecompass/SKILL.md` and `/discovery` before
falling back to direct `codecompass query`/SQL. Ledgerkit's own tests,
compat-register, and `knowledge/DECISIONS.md` remain authoritative
regardless of what CodeCompass surfaces.
